...
static void tcp_connect_to_server(void *arg);
...
{
    /* Initialize lwIP from thread */
    if (sys_thread_new("STE_tcp_connect", tcp_connect_to_server, NULL, INIT_THREAD_STACKSIZE, INIT_THREAD_PRIO) == NULL)
    {
        LWIP_ASSERT("STE_start_tcp_echo_client(): Task creation failed.", 0);
    }
}
...
/*!
 * @brief TCP client task for connecting to server.
 *
 * @param arg unused
 */
static void tcp_connect_to_server(void *arg)
{
    ...
    af = ip_port_to_sockaddr("192.168.0.100", 57777, &ipv4, &ipv6); // IP address of the server
...
static void echo_loop_tcp(int sck)
{
    ...
    while (1)
    {
        ...
        ssize_t bytes_sent = write(sck, message, message_len);
        if (bytes_sent >= 0)
        {
            PRINTF("Sent: %s\r\n", message);
        }
        ...
        ssize_t bytes_received = read(sck, buf, sizeof(buf));
        if (bytes_received > 0)
        {
